            ...
            <ul style="list-style: none; padding: 0; margin: 0;">
                ...
                <?php if ($is_admin || $is_sys_admin || $is_syndicate_admin): ?>
                    <li class="sm-sidebar-item <?php echo $active_tab == 'global-settings' ? 'sm-active' : ''; ?>">
                        ...
                        <ul class="sm-sidebar-dropdown" style="display: <?php echo $active_tab == 'global-settings' ? 'block' : 'none'; ?>;">
                            ...
                            <?php if ($is_sys_admin): ?>
                                <li><a href="<?php echo add_query_arg(['sm_tab' => 'global-settings', 'sub' => 'staff']); ?>" class="<?php echo ($_GET['sub'] ?? '') == 'staff' ? 'sm-sub-active' : ''; ?>"><span class="dashicons dashicons-admin-users"></span> إدارة مستخدمي النظام</a></li>
                                <li><a href="<?php echo add_query_arg(['sm_tab' => 'global-settings', 'sub' => 'alerts']); ?>" class="<?php echo ($_GET['sub'] ?? '') == 'alerts' ? 'sm-sub-active' : ''; ?>"><span class="dashicons dashicons-megaphone"></span> تنبيهات النظام</a></li>
                                <li><a href="<?php echo add_query_arg(['sm_tab' => 'global-settings', 'sub' => 'backup']); ?>" class="<?php echo ($_GET['sub'] ?? '') == 'backup' ? 'sm-sub-active' : ''; ?>"><span class="dashicons dashicons-database-export"></span> مركز النسخ الاحتياطي</a></li>
                                <li><a href="<?php echo add_query_arg(['sm_tab' => 'global-settings', 'sub' => 'emails']); ?>" class="<?php echo ($_GET['sub'] ?? '') == 'emails' ? 'sm-sub-active' : ''; ?>"><span class="dashicons dashicons-email"></span> إعدادات البريد التقني</a></li>
                                <li><a href="<?php echo add_query_arg(['sm_tab', 'global-settings', 'sub' => 'logs']); ?>" class="<?php echo ($_GET['sub'] ?? '') == 'logs' ? 'sm-sub-active' : ''; ?>"><span class="dashicons dashicons-list-view"></span> سجل النشاطات</a></li>
                            <?php endif; ?>
                        </ul>
                    </li>
                <?php endif; ?>
                ...
                <?php if ($is_sys_admin): ?>
                    <li class="sm-sidebar-item <?php echo $active_tab == 'branches' ? 'sm-active' : ''; ?>">
                        <a href="<?php echo add_query_arg('sm_tab', 'branches'); ?>" class="sm-sidebar-link"><span class="dashicons dashicons-networking"></span> إدارة الفروع</a>
                    </li>
                <?php endif; ?>
            </ul>
        </div>
                    ...
                    if ($is_admin || $is_sys_admin) {
                        ...
                        <div id="syndicate-branches-settings" class="sm-internal-tab" style="display: <?php echo ($sub == 'branches') ? 'block' : 'none'; ?>;">
                            <div style="background:#fff; border:1px solid #e2e8f0; border-radius:12px; padding:20px;">
                                <h4 style="margin:0 0 20px 0;">إدارة فروع ولجان النقابة (Branches Management)</h4>
                                <form method="post">
                                    <?php wp_nonce_field('sm_admin_action', 'sm_admin_nonce'); ?>
                                    <div class="sm-form-group">
                                        <label class="sm-label">قائمة فروع النقابة (فرع واحد في كل سطر):</label>
                                        <textarea name="sm_branches_list" class="sm-textarea" rows="10" placeholder="التنسيق: key|Label (مثال: cairo|فرع القاهرة)"><?php
                                            $branches = get_option('sm_branches', SM_Settings::get_governorates());
                                            foreach($branches as $k => $v) echo "$k|$v\n";
                                        ?></textarea>
                                        <p style="font-size:12px; color:#666; margin-top:5px;">يتم تعيين كل مسؤول نظام لفرع محدد، ولا يمكنه الوصول إلا لبيانات الأعضاء التابعين لهذا الفرع.</p>
                                    </div>
                                    <button type="submit" name="sm_save_branches_list" class="sm-btn" style="width:auto; height:45px; padding:0 30px;">حفظ قائمة الفروع</button>
                                </form>
                            </div>
                        </div>
                        ...
                        <div id="system-email-settings" class="sm-internal-tab" style="display: <?php echo ($sub == 'emails') ? 'block' : 'none'; ?>;">
                            <div style="background:#fff; border:1px solid #e2e8f0; border-radius:12px; padding:20px;">
                                ...
                                <form method="post">
                                    ...
                                    <div class="sm-form-group">
                                        <label class="sm-label">بريد الإرسال التلقائي (From/No-Reply):</label>
                ...
                case 'global-settings':
                    if ($is_admin || $is_sys_admin || $is_syndicate_admin) {
                        ...
                        <div class="sm-tabs-wrapper" style="display: flex; gap: 10px; margin-bottom: 20px; border-bottom: 2px solid #eee; overflow-x: auto; white-space: nowrap; padding-bottom: 10px;">
                            ...
                            <?php if ($is_sys_admin): ?>
                                <button class="sm-tab-btn <?php echo $sub == 'staff' ? 'sm-active' : ''; ?>" onclick="smOpenInternalTab('system-users-settings', this)">إدارة المستخدمين</button>
                                <button class="sm-tab-btn <?php echo $sub == 'alerts' ? 'sm-active' : ''; ?>" onclick="smOpenInternalTab('system-alerts-settings', this)">تنبيهات النظام</button>
    ...
    <div class="sm-modal-content" style="max-width: 600px;">
        ...
        <form id="sm-alert-form" style="padding: 20px;">
            ...
            <div style="background: #f8fafc; padding: 15px; border-radius: 8px; border: 1px solid #edf2f7; margin-bottom: 20px;">
                <label class="sm-label" style="color: var(--sm-primary-color); border-bottom: 1px solid #eee; padding-bottom: 5px; margin-bottom: 10px;">استهداف العرض (Targeting):</label>

                <div class="sm-form-group">
                    <label class="sm-label" style="font-size: 12px;">استهداف الأدوار (Roles):</label>
                ...
                <div class="sm-form-group">
                    <label class="sm-label" style="font-size: 12px;">استهداف الرتب (Ranks):</label>
